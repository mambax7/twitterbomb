...
class TwitterbombScheduler extends XoopsObject
{
    public function __construct($fid = null)
    {
        ...
        $this->initVar('mode', XOBJ_DTYPE_ENUM, 'direct', false, false, false, [
            'direct',
            'filtered',
            'pregmatch',
            'strip',
            ...
            'filteredpregmatch',
            'pregmatchfiltered',
            'filteredpregmatchstrip',
            'filteredstrippregmatch',
            'pregmatchfilteredstrip',
            'pregmatchstripfiltered',
            'strippregmatchfiltered',
            'stripfilteredpregmatch',
            'mirc',
        ]);
        $this->initVar('pre', XOBJ_DTYPE_TXTBOX, null, false, 35);
        $this->initVar('text', XOBJ_DTYPE_TXTBOX, null, true, 500);
        $this->initVar('search', XOBJ_DTYPE_ARRAY, [], false);
        $this->initVar('replace', XOBJ_DTYPE_ARRAY, [], false);
        $this->initVar('strip', XOBJ_DTYPE_ARRAY, ['[', ']', '{', '}', '|', '@', '#'], false);
    ...
    public function getTweet()
    {
        ...
        if (is_string($ret) && !is_object($ret)) {
            return ['sid' => $this->getVar('sid'), 'tweet' => ucfirst(trim((strlen($this->getVar('pre')) > 0 ? $this->getVar('pre') . ' ' : '') . $myts->censorString($ret)))];
        } else {
            if (is_object($ret)) {
                return ['sid' => $this->getVar('sid'), 'tweet' => ucfirst(trim((strlen($this->getVar('pre')) > 0 ? $this->getVar('pre') . ' ' : '') . $myts->censorString($ret->getVar('text'))))];
            } else {
                return '';
            }
        }
...
class TwitterbombSchedulerHandler extends XoopsPersistableObjectHandler
{
    ...
    public function getTweet($cid = '0', $catid = '0', $when = '0', $tweeted = '0')
    {
        $this->recalc();
    ...
    public function recalc()
    {
        ...
        if (0 != $this->_modConfig['number_to_rank']) {
            // Reset Rank
        ...
        // Kill Old Tweets
        if (0 != $this->_modConfig['kill_tweeted']) {
    ...
    public function getFilterForm($filter, $field, $sort = 'sid', $op = 'scheduler', $fct = 'list')
    {
